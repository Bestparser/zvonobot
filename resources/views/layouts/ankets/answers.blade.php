<div class="mt-2 font-bold text-center">Ответы на вопрос "{{ $question }}"</div>
<table class="usersTable mt-3 border border-gray-200 text-gray-800 w-full text-sm text-center">
    <thead>
        <tr class="h-10">
            ...
            <th class="sticky top-0 bg-blue-50">Ответ</th>
            <th class="sticky top-0 bg-blue-50">Сортировка</th>
            <th class="sticky top-0 bg-blue-50">Значение</th>
            <th class="sticky top-0 bg-blue-50">Ситуация</th>
    ...
    <tbody class="divide-y">                                    
        @foreach ($answers as $answer)
            <tr class="whitespace-nowrap callstr">
                <td class="p-1">{{ $answer->id }}</td>
                <td style="text-align: left;" class="p-1">{{ $answer->answer_name }}</td>
                <td class="p-1">{{ $answer->order }}</td>                
                <td class="p-1">{{ $answer->value }}</td>
                <td class="p-1">{{ $answer->situation }}</td>
                <td class="p-1">                    
                    @if ($answer->hidden == 0) Да @endif
                    @if ($answer->hidden == 1) Нет @endif
                </td>
                <td class="p-1">
                    <div class="flex flex-col space-y-2 flex-nowrap justify-center items-center">
                        <a href="ankets?editAnswerID={{ $answer->id }}" class="editUserButton padding-button inline-flex cursor-pointer justify-center items-center whitespace-nowrap focus:outline-none transition-colors focus:duration-150 border rounded ring-blue-700 p-2 hover:bg-gray-50 bg-white text-black border-gray-300 last:mr-0" type="button">
...
<button type="button" onclick="location.href='{{ $comebackURL }}'" type="button" class="px-4 py-1 mr-10 bg-gray-100 bg-transparent rounded-lg text-indigo-500 hover:bg-gray-200 hover:text-indigo-400">Назад</button>
